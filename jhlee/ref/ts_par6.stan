data {
  ...
  int<lower=0, upper=2> level1_choice[N,T];  // 1: left, 2: right
  int<lower=0, upper=4> level2_choice[N,T];  // 1-4: 1/2: commonly associated with level1=1, 3/4: commonly associated with level1=2
...
generated quantities {
  ...
  real v_mf_1c[N,T];
  real v_mf_1nc[N,T];
  real v_hybrid_1c[N,T];
  real v_hybrid_1nc[N,T];
  ...
  for (i in 1:N) {
    ...
    for (t in 1:Tsubj[i])  {
      ...
      // Store values for level1 model regressors before the update
      v_mb_1c[i,t] = v_mb[level1_choice[i,t]];
      v_mb_1nc[i,t] = v_mb[3-level1_choice[i,t]];
      v_mf_1c[i,t] = v_mf[level1_choice[i,t]];
      v_mf_1nc[i,t] = v_mf[3-level1_choice[i,t]];
      v_hybrid_1c[i,t] = v_hybrid[level1_choice[i,t]];
      v_hybrid_1nc[i,t] = v_hybrid[3-level1_choice[i,t]];
      
      // nuisance regressor for the first-stage onset
      if(level1_choice[i,t] == 1){
        p_1[i,t] = 1 - level1_prob_choice2;
      } else {
        p_1[i,t] = level1_prob_choice2;
      }
      
      p_1_diff[i,t] = beta1[i]*p_1[i,t]*(v_mb[level1_choice[i,t]] - v_mf[level1_choice[i,t]] - p_1[i,t]*(v_mb[level1_choice[i,t]] - v_mf[level1_choice[i,t]])-(1-p_1[i,t])*(v_mb[3-level1_choice[i,t]] - v_mf[3-level1_choice[i,t]]));
      
      // level1 prediction error
      pe_1[i,t] = v_mf[2+ level2_choice[i,t]] - v_mf[level1_choice[i,t]];
      pe_1_mb[i,t] = v_mf[2+ level2_choice[i,t]] - v_mb[level1_choice[i,t]];
      pe_1_diff[i,t] = -v_mb[level1_choice[i,t]] + v_mf[level1_choice[i,t]];
      ...
      // Store values for level2 model regressors before the update
      if (level2_choice[i, t] > 2){
        v_mf_2c[i,t] = v_mf[level2_choice[i,t]];
        v_mf_2nc[i,t] = v_mf[7-level2_choice[i,t]];
      } else {
        v_mf_2c[i,t] = v_mf[level2_choice[i,t]];
        v_mf_2nc[i,t] = v_mf[3-level2_choice[i,t]];
      }
      
      // level2 prediction error
      pe_2[i,t] = reward[i,t] - v_mf[2+ level2_choice[i,t]];
      pe_2_diff[i,t] = 0;
