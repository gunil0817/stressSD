...
####### Function #######
# M16 model 
extract_param <- function(pars_par6, data, save_dir){
  # Get information from data
  n_subj = length(unique(data_behav$subjID))
  trials <- data_behav %>% group_by(subjID) %>% summarise(count=n())
  t_max = max(trials$count)
  t_subjs = array(trials$count)
  
  SV <- array( 0, c(n_subj, t_max))
  SO <- array( 0, c(n_subj, t_max))
