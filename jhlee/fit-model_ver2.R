...
setwd(FOLDER_ROOT)
source("Bayesian_modeling/utils/loo_model.R") 
...
FILE_DATA = sprintf("%sdata/SD_120_whole_n41_SDlike.txt", FOLDER_MODEL)
FILE_MODEL = sprintf('%s%s.stan', FOLDER_STAN, NAME_MODEL)

FILE_OUTPUT = sprintf('%soutput_%s.RData', 'C:/Users/compu/Desktop/ToSend/stan_output/', NAME_MODEL)
FILE_MODEL_OUTPUT = sprintf('%sparam_output_%s.RData', 'C:/Users/compu/Desktop/ToSend/model_output/', NAME_MODEL)
# FILE_MODEL_OUTPUT = sprintf('%sparam_output_%s.RData', MODEL_OUTPUT, NAME_MODEL)

POI = c("mu_k", "mu_beta", "mu_eta", "mu_tau", 
        "k", "beta", "eta", "tau", "log_lik")
...
#synthetic subject task conditions
{
  ...
  SDLike           = array(0, c(nump, numTrial))
  ### 
...

{
  ...
  print(unique(social_distance[1,]))
}
...
sM16 <-  summary(output, pars=c("k","tau", "beta", "eta"), probs = c(0.1, 0.9))$summary

loo15 = loo_model(output)
loo16 = loo_model(output)
comp = loo_compare(loo15, loo16)
