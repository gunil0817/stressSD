...
transformed parameters{
  ...
  vector<lower=0, upper=2>[N] k;
  vector<lower=0, upper=14>[N] tau;
    
  for (i in 1:N) {
    k[i] = Phi_approx(mu_pr[1] + sigma[1] + k_pr[i]) * 2;
    tau[i] = Phi_approx(mu_pr[2] + sigma[2] + tau_pr[i]) * 14;    
  }
}
...
// Hyperbolic function
  ...
  for (i in 1:N){
    ...
    for (t in 1:(Tsubj[i])){
      ev_self = amount_self[i, t];
      ev_other = (amount_other[i, t] + amount_self[i, t] - amount_default[i, t]) / (1 + k[i] * social_distance[i, t]);
...
generated quantities {
  ...
  real<lower=0, upper=2> mu_k;
  real<lower=0, upper=14> mu_tau;
  ...
  mu_k    = Phi_approx(mu_pr[1]) * 2;
  mu_tau = Phi_approx(mu_pr[2]) * 14;
