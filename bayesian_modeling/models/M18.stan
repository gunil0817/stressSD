...
data {
  ...
  real<lower=0> SDLike[N, T]; // inequality aversion babbe
...
model {
  ...
  for (i in 1:N) {
    ...
    for (t in 1:(Tsubj[i])) {
      ...
      ev_other = (amount_other[i, t]) * exp(-1 * ( k[i] * SDLike[i, t])) - eta[i] * inequality[i, t];
...
generated quantities {
  ...
  { // local section, this saves time and space
    for (i in 1:N) {
      ...
      for (t in 1:(Tsubj[i])) {
        ev_self = (amount_self[i, t] - amount_default[i, t]) * beta[i];
        ev_other = (amount_other[i, t]) * exp(-1 * ( k[i] * SDLike[i, t]))  - eta[i] * inequality[i, t];
