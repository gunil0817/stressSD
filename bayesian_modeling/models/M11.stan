...
parameters {
  ...
  vector[N] beta_pr; //individual sensitivity
}
...
transformed parameters{
  ...
  for (i in 1:N) {
    ...
    beta[i] = Phi_approx(mu_pr[3] + sigma[3] + beta_pr[i] * 2);    
  }
}
...
// Hyperbolic function
  ...
  for (i in 1:N){
    ...
    for (t in 1:(Tsubj[i])){
      ev_self = (1-beta[i]) * (amount_self[i, t] - amount_default[i, t]);
      ev_other = (beta[i] * amount_other[i, t]) / (1 + k[i] * social_distance[i, t]);
