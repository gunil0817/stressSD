...
M17_simulator_SVnet = function(dataList, k, beta, tau, eta, nrep) {
  seed = 2022
  ...
  SVnet     <- array(0, c(1, numTrial))
    ...
    for (n in 1:nrep){
      for (t in 1:numTrial) {
        ...
        ev_other =  amount_other[1, t] / (1 + k * social_distance[i, t])  - eta * inequality[i, t]
...
M17_simulator_psplit = function(dataList, k, beta, tau, eta, nrep) {
  seed = 2022
  for (i in 1:dataList$N) {
    tmpData         = subset(dataList, dataList$subjID == i)
    amount_self     = dataList$amount_self
