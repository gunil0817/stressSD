#Genearting Fake data for M6. 
#DataList contains amount_self, amount_other, amount_default, social_distance
#For certain values of k and tau, computes the Subjective values of
...
hypM14_simulator = function(dataList, k, beta, tau, nrep) {
  ...
  SVnet <- array(0,  c(1, numTrial))
    ...
    for (n in 1:nrep){
      for (t in 1:numTrial) {
        ev_self  = beta*(amount_self[1,t])
        ev_other = (1-beta)*(amount_other[1,t] + amount_self[1,t] -  amount_default[1,t]) / (1 + k * social_distance[i,t])
