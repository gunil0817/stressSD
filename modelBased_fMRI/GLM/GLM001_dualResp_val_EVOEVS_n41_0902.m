...
dirDM = '\GLM001_R3_M14_twoResp_val_EVOEVS_n41_0322';
dirOnSetTime = baseDir;
dirScan = dirDM;
analysisType = 'twoTT';

condName = {'social', 'share', 'selfish'};
...
%parametricName = {'mSD','ev_self','ev_other', 'pSplit'};
parametricName = {'mSD','ev_self','ev_other','probChoice'};
%contrastName = {'social_mSD','S-I', 'S_SD', 'S_AS', 'I_SD','I_AS','m_SD','m_AS'};
%contrastName = {'social_mSD','share_other','share_self', 'selfish_other', 'selfish_self', ...
%    'shareDiff', 'shareEVO-selfEVO', 'selfishDiff','selfEVS-selfEVO', 'sharepSplit', 'selfpSelf','shaMinSelpSplit'};
% 
contrastName = {'split_EVOEVS', 'incent_EVSEVO','splitEVOEVS-incentEVSEVO'};
contrastVector{1} = [0 0  0 1  0 0 ];
contrastVector{2} = [0 0  0 0  0 1 ];
contrastVector{3} = [0 0  0 1  0 -1];

onsetList = dir('E:\StressTask\onsetFilesParamM14\*.mat');
%% 2nd_lvl setting
dirGroup = 'E:\StressTask\Round3\2ndResult\oneTT';
dirSingleName = strcat(dirDM, '_oneTT');
...
for numSubj = 1:length(onsetList)
    ...
    matlabbatch{1}.spm.stats.fmri_spec.sess.cond(1).pmod(1) = struct('name', parametricName{1}, 'param', onsetTime{12}, 'poly', 1);
    %matlabbatch{1}.spm.stats.fmri_spec.sess.cond(1).pmod(1) = struct('name', parametricName{2}, 'param', onsetTime{6}*-1, 'poly', 1);
    matlabbatch{1}.spm.stats.fmri_spec.sess.cond(1).orth = 0;
    

    EVOEVS = onsetTime{7}-onsetTime{8};
    EVSEVO = onsetTime{10}-onsetTime{9};
    matlabbatch{1}.spm.stats.fmri_spec.sess.cond(2).name = condName{2}; %split
    ...
    matlabbatch{1}.spm.stats.fmri_spec.sess.cond(2).pmod(1) = struct('name', 'EVO_EVS', 'param', EVOEVS, 'poly', 1);
    %matlabbatch{1}.spm.stats.fmri_spec.sess.cond(2).pmod(2) = struct('name', parametricName{3}, 'param', onsetTime{8}, 'poly', 1);
    %matlabbatch{1}.spm.stats.fmri_spec.sess.cond(2).pmod(3) = struct('name', parametricName{4}, 'param', onsetTime{14}, 'poly', 1);
    ...
    %matlabbatch{1}.spm.stats.fmri_spec.sess.cond(3).pmod(1) = struct('name', parametricName{2}, 'param', onsetTime{9}, 'poly', 1);
    matlabbatch{1}.spm.stats.fmri_spec.sess.cond(3).pmod(1) = struct('name','EVS_EVO', 'param', EVSEVO, 'poly', 1);
    %-1 was multiplied for inversion / psplit -> pselfish
    %matlabbatch{1}.spm.stats.fmri_spec.sess.cond(3).pmod(3) = struct('name', parametricName{4}, 'param', onsetTime{15}.*-1, 'poly', 1); 
