...
dirDM = '\GLM023_R4_M16_spin_dualResp_EVOnEVS_n41_0328'; % directory for 1st lvl glm
...
contrastVector{7} = [0   0 1 -1  0 -1 1];
contrastVector{8} = [0   1 0 0   0 0 0];
contrastVector{9} = [0   0 0 0   1 0 0];
contrastVector{10} = [0  1 0 0   -1 0 0];
...
for numSubj = 1:length(onsetList)
    ...
    matlabbatch{1}.spm.stats.fmri_spec.sess.cond(2).onset = onsetTime{:,6};
    ...
    matlabbatch{1}.spm.stats.fmri_spec.sess.cond(2).pmod(2) = struct('name', 'EVS', 'param', onsetTime{34}, 'poly', 1);
    ...
    matlabbatch{1}.spm.stats.fmri_spec.sess.cond(3).pmod(2) = struct('name', 'EVS', 'param', onsetTime{36}, 'poly', 1);
